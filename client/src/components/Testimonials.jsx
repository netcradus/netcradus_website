          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {testimonials.map((item, idx) => (
              <GlassCard
                key={idx}
                delay={idx * 0.1}
                className="p-10 border-[var(--border-color)] bg-surface-color/5 relative h-auto overflow-visible"
              >
                ...
                <p className="text-[var(--text-primary)] font-sans text-sm md:text-lg leading-relaxed mb-10 opacity-90 relative z-10 whitespace-normal break-words">
                  "{item.quote}"
                </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-10 place-items-center">
              ...
              {clientLogos.map((logo, idx) => (
                <img
                  ...
                  src={logo}
                  alt="Client logo"
                  className="h-16 md:h-20 w-auto object-contain opacity-80 hover:opacity-100 transition"
                />
              ))}
