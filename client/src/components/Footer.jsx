...
export default function Footer() {
  ...
  return (
    <footer className="relative z-10 overflow-hidden border-t border-[#1E293B] bg-[#02060F] pt-16 transition-colors duration-500">
      {/* Background Container: FOOTER.png Background (Slightly more visible & crisp) */}
      <div className="absolute inset-0 z-0 pointer-events-none select-none overflow-hidden">
        {/* Soft Blue Ambient Glow Behind Map */}
        <div className="absolute left-1/3 top-10 h-[350px] w-[500px] rounded-full bg-[#00B8FF]/12 blur-[130px]" />

        {/* FOOTER.png Background Image (55% opacity, 0.5px subtle blur, 90% brightness) */}
        <img
          src="/FOOTER.png"
          alt="Footer Cyber Background"
          className="w-full h-full object-cover opacity-55 brightness-90 contrast-100 blur-[0.5px]"
        />

        {/* Dark Edge Vignette Fade */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_30%,#02060F_95%)]" />

        {/* Soft Darkening Overlay for Text Contrast */}
        <div className="absolute inset-0 bg-[#02060F]/15" />
      </div>

      {/* Top Border Glow Accent Line */}
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#00B8FF]/40 to-transparent z-10 pointer-events-none" />

      <div className="relative z-10 mx-auto grid max-w-screen-2xl gap-10 px-4 pb-14 sm:px-6 lg:grid-cols-12 lg:px-16 xl:px-24 text-center sm:text-left">
        {/* Left Section */}
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:col-span-5 lg:grid-cols-3">
          {footerColumns.slice(0, 3).map((column) => (
            <div key={column.title}>
              <h4 className="mb-5 text-sm font-semibold uppercase tracking-[0.2em] text-[#FF7A00] drop-shadow-[0_2px_8px_rgba(0,0,0,0.45)]">
                {column.title}
              </h4>
              <ul className="space-y-3 text-sm">
                {column.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      to={link.to}
                      className="text-[#FFFFFF] font-medium transition hover:text-[#00B8FF] drop-shadow-[0_2px_8px_rgba(0,0,0,0.45)]"
                    >
                      {link.label}
                    </Link>
                  </li>
      ...
      {/* Bottom Bar */}
      <div className="relative z-10 border-t border-[#1E293B]/80 py-5">
        <div className="mx-auto flex max-w-screen-2xl flex-col gap-3 px-4 text-sm text-[#FFFFFF] font-medium drop-shadow-[0_2px_8px_rgba(0,0,0,0.45)] sm:px-6 md:flex-row md:items-center md:justify-between lg:px-16 xl:px-24">
          <p>Copyright © 2026 Netcradus. All rights reserved.</p>

          <div className="flex flex-wrap gap-4">
            <Link to="/policy" className="transition hover:text-[#00B8FF]">
              Privacy Policy
            </Link>
            <Link to="/terms" className="transition hover:text-[#00B8FF]">
              Terms of Service
            </Link>
            <Link to="/cookies" className="transition hover:text-[#00B8FF]">
              Cookie Policy
            </Link>
            <Link
              to="/responsible-disclosure"
              className="transition hover:text-[#00B8FF]"
            >
