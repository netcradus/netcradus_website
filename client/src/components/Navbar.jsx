...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Partners', path: '/partners' },
    { name: 'About', path: '/about' },
    // where.exe git{ name: 'Gallery', path: '/gallery' },
    { name: 'Blog', path: '/blog' },
    { name: 'Contact', path: '/contact' },
  ];
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled glass' : ''}`}>
      <div className="container max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-16 xl:px-24 flex items-center justify-between h-20">
        ...
        <div className="nav-actions flex items-center gap-4">
          ...
          >
            {theme === 'dark'
              ? <Sun size={18} className="text-accent-bright" />
          ...
          <a
            ...
            className="btn-primary btn-glow-pulse desktop-only px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg flex items-center gap-2"
          >
            <Phone size={13} className="animate-wiggle" />
            Call Toll-Free <br />
            1800 12100 8800
